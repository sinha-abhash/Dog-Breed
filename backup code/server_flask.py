from test_bulbs import CVModel as BulbModel, main as bulb_main, results as bulb_results
from test_bags import CVModel as BagModel, main as bag_main, results as bag_results
...
def download_image(url):
    ...
    if resp.status_code == 200:
        ...
        return None
def slack_bot_message(fid,confidence,valve,name):
    fid = str(fid)
    return {
            "text": "Product Recognition Bot could not recognize the product. Please assist.",
            "attachments": [
                {
                    "text": "Engine was {0}% sure that it was a {1} valve".format(confidence,valve),
                    ...
                    "actions": [
                        {
                            ...
                            "text": "T2",
                            "type": "button",
                            "value": "T2_"+fid+"_"+name
                        },
                        {
                            "name": "valve",
                            "text": "TE5",
                            "type": "button",
                            "value": "TE5_"+fid+"_"+name
                        },
                        {
                            "name": "no valve",
                            "text": "Not a Valve",
...
def process_image_bulb():
    ...
    if not url:
        ...
        while not bulb_results.get(fid,None):
            time.sleep(1)
        valve_type,confidence = bulb_results.get(fid,None)[0]
        del bulb_results[fid]
        return Response(json.dumps({"response":valve_type},ensure_ascii=False),mimetype='application/json')
...
def process_image_bag():
    ...
    if not url:
        ...
        local_path = download_image(url)
        resp = bag_main(fid,local_path)
        while not bag_results.get(fid,None):
            time.sleep(1)
        valve_type,confidence = bag_results.get(fid,None)[0]
        del bag_results[fid]
        return Response(json.dumps({"response":valve_type},ensure_ascii=False),mimetype='application/json')
...
def process_image():
    ...
    if not url:
        ...
        local_path = download_image(url)
        resp = main(fid,local_path)
        while not test_results.get(fid,None):
            time.sleep(1)
        valve_type,confidence = test_results.get(fid,None)[0]
        del test_results[fid]
        if confidence < 80:
            message = slack_bot_message(fid,confidence,valve_type,user_name)
            ...
            if post_to_sf:
                response = SfObject().create_case(url,fid,user_name,confidence,valve_type)
...
def slack():
    ...
    valve_type,fid,name = actions.split("_")
    url = di['original_message']['attachments'][0]['image_url']
    queue.append(("slack",(valve_type,fid,url)))
...
def slack_post_to_messenger(url=None,fid=None,valve=None):
    bot_url = "https://dev.motherbot.co/danfossmodel3/custom/image_manual_response"
    response = requests.post(bot_url,json={"image_url":url,"valve_type":valve,"user_id":fid})
    print response
    return response
...
def salesforce_post_to_messenger(comment,fid=None,caseid=None):
    bot_url = "https://dev.motherbot.co/danfossmodel3/custom/image_manual_response"
    response = requests.post(bot_url,json={"valve_type":comment,"url":fid,"user_id":fid})
    print response
    return response
    ...
    def run(self):
        while True:
            if len(queue)>0:
                ...
                if typ == 'slack':
                    valve, fid, url = tpl
                    response = slack_post_to_messenger(url,fid,valve)
                else:
